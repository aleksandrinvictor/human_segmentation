...
def encode_rle(mask):
    ...
    ----------
    mask : np.ndarray, 2d
        Mask that consists of 2 unique values: 0 - denotes background,
        1 - denotes object.
    Returns
    -------
    str
    ...
    Notes
    -----
    Mask should contains only 2 unique values, one of them must be 0,
    another value, that denotes object, could be different from 1
    (for example 255).
    """
    pixels = mask.flatten()
...
def decode_rle(rle_mask, shape=(320, 240)):
    ...
    -------
    np.ndarray, 2d
        Mask that contains only 2 unique values:
