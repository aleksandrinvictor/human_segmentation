...
def generate_html(path_to_data):
    ...
    ----------
    path_to_data : str
        Path to data with original images, predicted masks, and cropped
        according masks images.
    Returns
    -------
    str
    ...
    paths_to_imgs = sorted(
        [
            "/".join(path.split("/")[-2:])
            for path in glob(f"{path_to_data}/*_img.jpg")
        ]
    )
    paths_to_masks = sorted(
        [
            "/".join(path.split("/")[-2:])
            for path in glob(f"{path_to_data}/*_pred_mask.png")
        ]
    )
    paths_to_crops = sorted(
        [
            "/".join(path.split("/")[-2:])
            for path in glob(f"{path_to_data}/*_crop.png")
        ]
    )
    for ind, (path_to_img, path_to_mask, path_to_crop) in enumerate(
        zip(paths_to_imgs, paths_to_masks, paths_to_crops)
    ):
        if not ind % 2:
            html += "<tr>\n"
...
def get_html(paths_to_imgs, pred_masks, path_to_save="results/test"):
    ...
    pred_masks : list[np.ndarray]
        Predicted masks.
    path_to_save : str
        Path to save source images to put them in html file.
        Html name is the same as name of the
    ...
    for path_to_img, pred_mask in zip(paths_to_imgs, pred_masks):
        ...
        Image.fromarray(pred_mask).convert("RGB").save(
            f"{path_to_save}/{img_id}_pred_mask.png"
        )
        crop_img = img.copy()
