...
class FeaturePyramidAttention(nn.Module):
    def __init__(self, in_channels: int):
        ...
        # Pyramid block
        self.conv_main = ConvBatchNormBlock(
            in_channels, kernel_size=1, stride=1
        )

        self.conv7x7_1 = ConvBatchNormBlock(
            in_channels, kernel_size=7, stride=1, padding=3
        )

        self.conv7x7_2 = ConvBatchNormBlock(
            in_channels, kernel_size=7, stride=1, padding=3
        )

        self.conv5x5_1 = ConvBatchNormBlock(
            in_channels, kernel_size=5, stride=1, padding=2
        )

        self.conv5x5_2 = ConvBatchNormBlock(
            in_channels, kernel_size=5, stride=1, padding=2
        )

        self.conv3x3_1 = ConvBatchNormBlock(
            in_channels, kernel_size=3, stride=1, padding=1
        )

        self.conv3x3_2 = ConvBatchNormBlock(
            in_channels, kernel_size=3, stride=1, padding=1
        )

        # Upsample
        self.upsample_1 = ConvBatchNormBlock(
            in_channels, kernel_size=3, stride=2, padding=1, upsample=True
        )

        self.upsample_2 = ConvBatchNormBlock(
            in_channels, kernel_size=3, stride=2, padding=1, upsample=True
        )

        self.upsample_3 = ConvBatchNormBlock(
            in_channels, kernel_size=3, stride=2, padding=1, upsample=True
        )

        # Global pooling branch
        self.gb_conv = ConvBatchNormBlock(
